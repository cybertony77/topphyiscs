...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { homeworkId, answers, questions } = req.body;

    if (!homeworkId || !answers || !questions) {
    ...
    // Get homework with correct answers
    const homework = await db.collection('homeworks').findOne({ _id: new ObjectId(homeworkId) });
    ...
    questions.forEach((qData, idx) => {
      const originalQ = homework.questions.find(q => 
    ...
    console.error('Validate Homeworks API error:', error);
