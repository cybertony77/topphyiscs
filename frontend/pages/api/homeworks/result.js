...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Get homework with correct answers for result display
    const homework = await db.collection('homeworks').findOne({ _id: new ObjectId(id) });
    
    if (!homework) {
      return res.status(404).json({ error: 'Homework not found' });
    }

    // Return homework with correct answers (for result page only)
    ...
    console.error('Result Homeworks API error:', error);
