...
export default function MonthlySubscription() {
  ...
  const createSubscriptionMutation = async (overwrite = false) => {
    try {
      ...
      if (result.success) {
        setSuccess(`Successfully subscribed ${duration} month${duration > 1 ? 's' : ''}`);
        setDuration(1);
        setCost('');
        setNote('');
      }
      ...
      <div className="page-container">
        <Title backText="Back" href="/subscription_dashboard">
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <Image src="/calendar3.svg" alt="Monthly" width={32} height={32} />
        ...
        <div className="subscription-form-container">
          <form onSubmit={handleSubmit} className="subscription-form">
            <div className="form-group">
              <label className="form-label">Duration (Months) <span className="required-asterisk">*</span></label>
              <DurationSelect
                selectedValue={duration}
                onValueChange={(value) => setDuration(value ? parseInt(value) : 1)}
                options={months.map(month => ({ value: month }))}
                placeholder="Select months"
                formatOption={(val) => `${val} ${val === 1 ? 'Month' : 'Months'}`}
              />
            </div>
