...
export default function YearlySubscription() {
  ...
  const createSubscriptionMutation = async (overwrite = false) => {
    try {
      ...
      if (result.success) {
        setSuccess(`Successfully subscribed ${duration} year${duration > 1 ? 's' : ''}`);
        setDuration(1);
        setCost('');
        setNote('');
      }
      ...
      <div className="page-container">
        <Title backText="Back" href="/subscription_dashboard">
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <Image src="/calendar.svg" alt="Yearly" width={32} height={32} />
            Yearly Subscription
          </div>
        </Title>
        ...
        <div className="subscription-form-container">
          <form onSubmit={handleSubmit} className="subscription-form">
            <div className="form-group">
              <label className="form-label">Duration (Years) <span className="required-asterisk">*</span></label>
              <DurationSelect
                selectedValue={duration}
                onValueChange={(value) => setDuration(value ? parseInt(value) : 1)}
                options={years.map(year => ({ value: year }))}
                placeholder="Select years"
                formatOption={(val) => `${val} ${val === 1 ? 'Year' : 'Years'}`}
              />
            </div>
