...
export default function MinutelySubscription() {
        ...
        <div className="subscription-form-container">
          <form onSubmit={handleSubmit} className="subscription-form">
            <div className="form-group">
              <label className="form-label">Duration (Minutes) <span className="required-asterisk">*</span></label>
              <DurationSelect
                selectedValue={duration}
                onValueChange={(value) => setDuration(value ? parseInt(value) : 1)}
                options={minutes.map(minute => ({ value: minute }))}
                placeholder="Select minutes"
                formatOption={(val) => `${val} ${val === 1 ? 'Minute' : 'Minutes'}`}
              />
            </div>
