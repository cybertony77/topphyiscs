...
export default async function handler(req, res) {
  ...
  const { attendanceLesson } = req.body;
  if (attendanceLesson === undefined) return res.status(400).json({ error: 'attendanceLesson required' });
  ...
  try {
    ...
    if (error.message.includes('Unauthorized') || error.message.includes('Invalid token')) {
      ...
      console.error('Error updating attendance lesson:', error);
