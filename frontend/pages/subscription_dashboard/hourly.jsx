...
export default function HourlySubscription() {
  ...
  const createSubscriptionMutation = async (overwrite = false) => {
    try {
      ...
      if (result.success) {
        setSuccess(`Successfully subscribed ${duration} hour${duration > 1 ? 's' : ''}`);
        setDuration(1);
        setCost('');
        setNote('');
      }
      ...
      <div className="page-container">
        <Title backText="Back" href="/subscription_dashboard">
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <Image src="/clock2.svg" alt="Hourly" width={32} height={32} />
            Hourly Subscription
          </div>
        </Title>
        ...
        <div className="subscription-form-container">
          <form onSubmit={handleSubmit} className="subscription-form">
            <div className="form-group">
              <label className="form-label">Duration (Hours) <span className="required-asterisk">*</span></label>
              <DurationSelect
                selectedValue={duration}
                onValueChange={(value) => setDuration(value ? parseInt(value) : 1)}
                options={hours.map(hour => ({ value: hour }))}
                placeholder="Select hours"
                formatOption={(val) => `${val} ${val === 1 ? 'Hour' : 'Hours'}`}
              />
            </div>
