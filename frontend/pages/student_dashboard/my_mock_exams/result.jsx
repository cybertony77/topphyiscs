...
export default function MockExamResult() {
  ...
  useEffect(() => {
    if (!id || !profile?.id) {
      if (!id) router.push('/student_dashboard/my_mock_exams');
      return;
    }
    ...
    const fetchResults = async (retryCount = 0) => {
      try {
        ...
        // Get mock exam and saved result from database
        ...
        if (response && response.data) {
          const me = response.data.mockExam;
          const savedResult = response.data.result;
          const hasResult = response.data.hasResult;
          
          // If no result found, redirect to mock exam list
          if (!hasResult || !savedResult) {
            console.log('No mock exam result found, redirecting...');
            setIsLoading(false);
          ...
          if (!me) {
            console.error('Missing mock exam data');
            setIsLoading(false);
            router.push('/student_dashboard/my_mock_exams');
            return;
          }
          
          // Validate mock exam has questions array
          if (!me.questions || !Array.isArray(me.questions) || me.questions.length === 0) {
            console.error('Mock exam missing questions array:', me);
            setIsLoading(false);
            router.push('/student_dashboard/my_mock_exams');
            return;
          }
          ...
          if (me.questions && Array.isArray(me.questions)) {
            me.questions.forEach((originalQ, questionIdx) => {
              ...
              const questionText = originalQ.question_text || originalQ.question || '';
            ...
            console.error('Mock exam questions array is missing or invalid:', me);
          }
          ...
          // Remove all mock exam-related sessionStorage items that were saved during start page
          if (id) {
            sessionStorage.removeItem(`mock_exam_${id}_answers`);
            ...
            sessionStorage.removeItem(`mock_exam_${id}_date_of_end`);
        ...
        // If still 404 after retries, or other error, redirect to mock exams list
        if (err.response?.status === 404) {
          console.log('Mock exam result not found, redirecting to mock exams list');
          router.push('/student_dashboard/my_mock_exams');
          return;
        }
    ...
    return (
      <div style={{ 
        ...
        <div style={{ maxWidth: 800, margin: "40px auto", padding: "20px 5px 20px 5px" }}>
          <Title backText="Back" href="/student_dashboard/my_mock_exams">Mock Exam Results</Title>
