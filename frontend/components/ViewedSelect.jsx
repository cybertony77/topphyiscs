...
const ViewedSelect = ({ 
  ...
  const options = [
    ...
    { value: true, label: '✅ Yes', color: '#28a745' },
    { value: false, label: '❌ No', color: '#dc3545' }
  ];
    ...
    <div className="form-group" style={{ ...style, marginBottom: '16px', textAlign: 'left' }}>
      {!style.hideLabel && (
        <label style={{ textAlign: 'left', display: 'block', marginBottom: '8px', fontWeight: 600, color: '#495057', fontSize: '0.95rem' }}>
          Viewed {required && <span style={{color: 'red'}}>*</span>}
        </label>
      )}
      <div 
        ref={dropdownRef}
      ...
      >
        ...
        {isOpen && (
          <div style={{
            ...
            {options.map((option) => (
              <div
                key={option.value === '' ? 'clear' : option.value.toString()}
                style={{
                  padding: '12px 16px',
                  cursor: 'pointer',
